...
export async function UpdatePointById(request: { pointId: any; data: any }): Promise<Response> {
    const response = new Response();
    const pointId = vf.IsNumeric(request) ? parseInt(request.pointId) : null;
    if (!pointId) {
        response.set(422, 'Invalid datatype for point id', null);
        return response;
    }
    if (!vf.IsAlpha(request.data.pointName) || !vf.IsAlphanumeric(request.data.pointDescription)) {
        response.set(422, 'Invalid datatype for point name and/or point description', null);
        return response;
    }
    try {
...
export async function DeletePointById(request: any): Promise<Response> {
    const response = new Response();
    const pointId = vf.IsNumeric(request) ? parseInt(request) : null;
    if (!pointId) {
        response.set(422, 'Invalid datatype for point id', null);
